package by.it.group251052.ramazanov.lesson03;
...
public class A_Huffman {
    ...
    //!!!!!!!!!!!!!!!!!!!!!!!!!     НАЧАЛО ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!!
    String encode(File inputFile) throws FileNotFoundException {
        ...
        for (int i = 0; i < inputString.length(); i++) {
            if (!characterCount.containsKey(inputString.charAt(i))) {
                ...
                characterCount.put(inputString.charAt(i), characterCount.get(inputString.charAt(i)) + 1);
            }
        }
        ...
        while (nodePriorityQueue.size() > 1) {
            Node leftNode = nodePriorityQueue.poll();
            Node rightNode = nodePriorityQueue.poll();
            InternalNode internalNode = new InternalNode(leftNode, rightNode);
            nodePriorityQueue.add(internalNode);
        }
        nodePriorityQueue.poll().fillCodes("");
