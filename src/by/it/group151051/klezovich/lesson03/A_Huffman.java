package by.it.group151051.klezovich.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        {
            char ch = s.charAt(i);
            if (count.containsKey(ch) == true)
            {
                int value = count.get(ch);
                value++;
                count.replace(ch, value);
            }
            else count.put(ch, 1);
        }
        ...
        for (Map.Entry<Character, Integer> enter : count.entrySet())
        {
            char letter = enter.getKey();
            int freq = enter.getValue();
            priorityQueue.add(new LeafNode(freq, letter));
        }
        ...
        while (priorityQueue.size() >= 2)
        {
            ...
            InternalNode predok = new InternalNode(Left, Right);
            priorityQueue.add(predok);
        }
