package by.it.group151051.krupovich.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i=0;i<s.length();i++)
        {
            char x= s.charAt(i);
            int cnt=0;
            if(count.containsKey(x)) cnt=count.get(x);
            count.put(x, ++cnt);
        }
       ...
       while(priorityQueue.size()>1){
            Node child=new InternalNode(priorityQueue.poll(), priorityQueue.poll());
