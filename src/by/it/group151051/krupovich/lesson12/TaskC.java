package by.it.group151051.krupovich.lesson12;
...
public class TaskC
{
    ...
    public static void Dijkstras_algorithm(int temp)
    {
    ...
    public static int findShortestCycle()
    {
        int result = 0;

        for (int i = 0; i < graph.get(u).size(); i++)
            if (v == graph.get(u).get(i).dest)
                result = graph.get(u).get(i).cost;

        return result + dist[u];
    }
    ...
    public static void main(String[] args)
    {
        ...
        Dijkstras_algorithm(v);

        System.out.print("The smallest length of the cycle between " + (char)(u + START_VERTEX) + " and " + (char)(v + START_VERTEX) + " = " + findShortestCycle());
    }
}