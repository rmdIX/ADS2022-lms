package by.it.group151051.makovetskiy.lesson08;
...
public class C_Stairs {
    ...
    int goUP(int step, int sum, int a[], int n)
    {
        if(step + 1 < n)  // можем шагнуть на две
        {
            if(a[step] >= 0 || a[step] > a[step + 1])
            {
                return goUP(step + 1, sum + a[step],a,n);
            }
            return goUP(step + 2, sum + a[step + 1],a,n);
        }
