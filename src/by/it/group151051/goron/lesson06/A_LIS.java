...
public class A_LIS {
    int getSeqSize(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // Бинарный поиск среди последних элементов найденных последовательностей, минимального элемента,
            // который больше чем i-й элемент исходного массива.
            // Это даст нам последний элемент найденной раньше последовательности, с которой будем работать дальше
            while (low <= high) {
                int mid = (low + high) >>> 1;
                if (X[M[mid]] > X[i])
                    high = mid - 1;
