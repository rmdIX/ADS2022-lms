package by.it.group251051.birukov.lesson05;
...
public class C_QSortOptimized {
    ...
    static int Q_count(Segment[] a, int left, int right, int point)
    {
        ...
        while (left <= right)
        {
            ...
            left_c = left;
            right_c = right;
            Segment pivot = a_c[left_c];
            while(left_c < right_c){
                left_c++;

                while(left_c <= right_c && a_c[left_c].compareTo(pivot) < 0){
                    left_c++;
                }
                while(right_c >= left_c && a_c[right_c].compareTo(pivot) < 0){
                    right_c--;
                }
                if(left_c < right_c){
                    Segment temp = a_c[left_c];
                    a_c[left_c] = a_c[right_c];
                    a_c[right_c] = temp;
                }
            }
            int pi = left_c;
